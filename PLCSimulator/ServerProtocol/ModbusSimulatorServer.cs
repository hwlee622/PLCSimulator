        ...
        private void SyncData()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    for (int i = 0; i < prevCoils.Length; i++)
                    {
                        var coil = m_server.coils[i];
                        if (prevCoils[i] != coil)
                            DataManager.Instance.BitDataDict[COIL].SetData(i, new bool[] { coil });
                        prevCoils[i] = coil;
                    }

                    for (int i = 0; i < prevHRegister.Length; i++)
                    {
                        var register = m_server.holdingRegisters[i];
                        if (prevHRegister[i] != register)
                            DataManager.Instance.WordDataDict[HOLDING_REGISTER].SetData(i, new ushort[] { (ushort)register });
                        prevHRegister[i] = register;
                    }

                    for (int i = 0; i < prevBits.Length; i++)
                    {
                        var bit = DataManager.Instance.BitDataDict[COIL].GetData(i, 1)[0];
                        if (prevBits[i] != bit)
                            m_server.coils[i] = bit;
                        prevBits[i] = bit;
                    }
                    ...
                    for (int i = 0; i < prevWords.Length; i++)
                    {
                        ...
                        if (prevWords[i] != word)
                            m_server.holdingRegisters[i] = (short)word;
                        prevWords[i] = word;
                    }
                }
                catch (Exception ex)
